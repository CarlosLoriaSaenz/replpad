...
module.exports = function commands(repl) {
  ...
  function updateCompact() {
	...
	if (repl.commands['.compact'])
        repl.commands['.compact'].help = compactHelp();
	else {
		repl.commands['.compact'] = commands.compact;
	}
  }
  ...
  function updateHighlight() {
	...
	if (repl.commands['.highlight'])
	    repl.commands['.highlight'].help = highlightHelp();
	else {
		repl.commands['.highlight'] = commands.highlight;
	}
  }
  ...
  function updateDepth() {
	...
	if (repl.commands['.depth'])
       repl.commands['.depth'].help = depthHelp();
    else {
		repl.commands['.depth'] = commands.depth;
	}
  }
  ...
  function updateHidden() {
	...
	if (repl.commands['.hidden'])
       repl.commands['.hidden'].help = hiddenHelp();
    else {
		repl.commands['.hidden'] = commands.hidden;
	}
  }
  ...
  function updateTalk() {
	...
	if(repl.commands['.talk'])
       repl.commands['.talk'].help = talkHelp();
    else {
		repl.commands['.talk'] = commands.talk;
	}
  }
